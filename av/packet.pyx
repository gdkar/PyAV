...
cdef class Packet(Buffer):
    ...
    cpdef void ref(self, Packet other):
        ...
        if other is not None:
            optr = other._ptr
        cdef lib.AVPacket * ptr  = self._ptr
        cdef int err = 0
        if ptr != optr:
            self.unref()
            if optr != NULL:
    ...
    property pos:
        def __get__(self): return None if self._ptr.pos == - \
            1 else self._ptr.pos
    property size:
        def __get__(self): return self._ptr.size
    property duration:
        def __get__(
            self): return None if self._ptr.duration == lib.AV_NOPTS_VALUE else self._ptr.duration
    ...
    property is_corrupt:
        def __get__(self): return bool(
            self._ptr.flags & lib.AV_PKT_FLAG_CORRUPT)
