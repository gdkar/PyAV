...
cdef object to_avrational(object value, lib.AVRational *input):
    ...
    if isinstance(value, Fraction):frac = value
    else:                          frac = Fraction(value)
...
cdef ByteSource bytesource(obj, bint allow_none=False):
    if allow_none and obj is None:return
    elif isinstance(obj, ByteSource):return obj
    else:return ByteSource(obj)
