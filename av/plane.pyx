...
cdef class Plane(object):
    ...
    def __repr__(self):    return '<av.%s at 0x%x>' % (self.__class__.__name__, id(self))
    def __len__(self):     return self.buffer_size
    property line_size:
        """Bytes per horizontal line in this plane."""
        def __get__(self): return self.frame.ptr.linesize[self.index]
    property ptr:
        def __get__(self): return <long>self.frame.ptr.extended_data[self.index]
    def to_bytes(self):    return <bytes>(<char*>self.frame.ptr.extended_data[self.index])[:self.buffer_size]
    def update(self, input):
        """Replace the data in this plane with the given buffer."""
        cdef ByteSource source = bytesource(input)
        if source.length != self.buffer_size: raise ValueError('got %d bytes; need %d bytes' % (len(input), self.buffer_size))
    ...
    def __getsegcount__(self, Py_ssize_t *len_out):
        if len_out != NULL: len_out[0] = <Py_ssize_t>self.buffer_size
        return 1

    def __getreadbuffer__(self, Py_ssize_t index, void **data):
        if index: raise RuntimeError("accessing non-existent buffer segment")
