    ...
    def demux(self, *args, **kwargs):
        ...
        try:
            ...
            while True:
                ...
                try:
                    with nogil:
                        ret = lib.av_read_frame(self.proxy.ptr, packet._ptr)
                    self.proxy.err_check(ret)
                except AVError:
                    break

                if include_stream[packet._ptr.stream_index]:
                    # If AVFMTCTX_NOHEADER is set in ctx_flags, then new streams
                    # may also appear in av_read_frame().
                    # http://ffmpeg.org/doxygen/trunk/structAVFormatContext.html
                    # TODO: find better way to handle this
                    if packet._ptr.stream_index < len(self.streams):
                        packet._stream = self.streams[packet._ptr.stream_index]
