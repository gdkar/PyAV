...
cdef class OutputContainer(Container):
    ...
    def mux_one(self, Packet packet not None):
        ...
        if packet._ptr.stream_index < 0 or packet._ptr.stream_index >= self.proxy.ptr.nb_streams:
            raise ValueError('Bad Packet stream_index.')
        cdef lib.AVStream *stream = self.proxy.ptr.streams[packet._ptr.stream_index]
        packet._rebase_time(stream.time_base)

        self.proxy.err_check(lib.av_interleaved_write_frame(self.proxy.ptr, packet._ptr))
