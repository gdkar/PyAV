...
cdef extern from "libavcodec/avcodec.h" nogil:
    cdef enum AVCodecID:
        pass
    ctypedef struct AVCodecDescriptor:
        AVCodecID     id
        AVMediaType type
        const char      *name
    ...
    cdef const uint64_t AV_CODEC_PROP_uint64_tRA_ONLY
    cdef const uint64_t AV_CODEC_PROP_LOSSY
    cdef const uint64_t AV_CODEC_PROP_LOSSLESS
    cdef const uint64_t AV_CODEC_PROP_REORDER
    cdef const uint64_t AV_CODEC_PROP_BITMAP_SUB
    cdef const uint64_t AV_CODEC_PROP_TEXT_SUB
    cdef const uint64_t AV_INPUT_BUFFER_PADDING_SIZE
    cdef const uint64_t AV_INPUT_BUFFER_MIN_SIZE
    cdef const uint64_t FF_INPUT_BUFFER_PADDING_SIZE
    cdef const uint64_t FF_MIN_BUFFER_SIZE
    cdef enum Motion_Est_ID:
        pass
    cdef enum AVDiscard:
        AVDISCARD_NONE
        AVDISCARD_DEFAULT 
