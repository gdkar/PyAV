...
def frame_iter(video):
    count = 0
    streams = [s for s in video.streams if s.type == 'video']
    streams = [streams[0]]
    for packet in video.demux(streams):
        for frame in packet.decode():
            yield frame
            count += 1
            if args.frames and count > args.frames:return
for src_path in args.path:
    ...
    for chunk_i in itertools.count(1):
        chunk = None
        for frame_i, frame in zip(list(range(max_size)), frames):
            if chunk is None:chunk = Image.new("RGB", (max_size, frame.height))
            img = frame.to_image()
        ...
        if chunk is None:
            ...
            if chunk.size[0] != (chunk_i + 1):chunk = chunk.crop((0, 0, frame_i + 1, chunk.size[1]))
